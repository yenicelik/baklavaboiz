...
best_model = None
for k in ks:
    rfc = AdaBoostClassifier(n_estimators=k)
    ...
    if acc > best_acc:
        best_model = rfc
        best_acc = acc
        print("New best AdaBoostClassifier model for k: {} and acc: {}".format(k, acc))
...
savefig('AdaBoostClassifier.png', bbox_inches='tight')
plt.show()
print("Done!")
sys.exit(0)
